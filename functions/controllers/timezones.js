...
timezoneRouter.get("/:username/zones", async (request, response) => {
	...
	const zones = await Timezone.find({ user: decodedToken.id }).populate(
		"user",
		{ username: 1, name: 1 }
	);
	let zoneArr = zones.map((zone) =>
		DateTime.local().setZone(zone.timezone).toString()
	);
	response.json(zones);
});
...
timezoneRouter.delete("/:username/zones/:zoneId", async (request, response) => {
	...
	if (tzone.user.toString() === decodedToken.id) {
		...
		return response
			.status(401)
			.json({ error: "Unauthorized to access the timezone" });
	}
});
